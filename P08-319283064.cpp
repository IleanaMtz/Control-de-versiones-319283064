...
int main()
{
	...
	Pastel = Model();
	Pastel.LoadModel("Models/pastel.obj");
	...
	//Declaración de primer luz puntual - Arreglo Lampara
	...
	//Declaración de primer luz puntual - Arreglo Pastel Lampara
	pointLights1[0] = PointLight(1.0f, 1.0f, 0.0f,
		0.5f, 0.5f,
		10.5f, 1.0f, -4.0f,
		0.3f, 1.0f, 0.1f);
	pointLightCount1++;
	...
	while (!mainWindow.getShouldClose())
	{
		...
		glm::vec3 luzCarro = glm::vec3(0.0f, -1.0f, -5.0f + (mainWindow.getAvanzarCarro() + mainWindow.getRetrocederCarro()) * 0.05);
		glm::vec3 lightDirCarro = glm::vec3(0.0f, 0.0f, 1.0f);
		glm::mat4 rotacion = glm::rotate(glm::mat4(1.0f), glm::radians(mainWindow.getCofre()), glm::vec3(1.0f, 0.0f, 0.0f));
		glm::vec3 lightDirCofre = glm::vec3(rotacion * glm::vec4(lightDirCarro, 0.0f));
		spotLights[0].SetFlash(luzCarro, lightDirCofre);

		//Luz carro adelante

		glm::vec3 luzCarro1 = glm::vec3(0.0f, -0.5f, -6.0 + (mainWindow.getAvanzarCarro() + mainWindow.getRetrocederCarro()) * 0.05);
		glm::vec3 lightDirCarro1 = glm::vec3(0.0f, 0.0f, 1.0f);
		spotLights1[0].SetFlash(luzCarro1, lightDirCarro1);

		//Luz carro atrás

		glm::vec3 luzCarro2 = glm::vec3(0.0f, -0.5f, -6.0 + (mainWindow.getAvanzarCarro() + mainWindow.getRetrocederCarro()) * 0.05);
		glm::vec3 lightDirCarro2 = glm::vec3(0.0f, 0.0f, -1.0f);
		spotLights2[0].SetFlash(luzCarro2, lightDirCarro2);
		...
		if (mainWindow.getLucesCarro() == 0) {
			shaderList[0].SetSpotLights(spotLights, spotLightCount);
		}
		else if (mainWindow.getLucesCarro() == 1) {
			shaderList[0].SetSpotLights(spotLights1, spotLightCount1);
		}
		else if (mainWindow.getLucesCarro() == 2) {
			shaderList[0].SetSpotLights(spotLights2, spotLightCount2);
		}

		if (mainWindow.getLuminaria() == 1 && mainWindow.getPastel() == 1) {
			shaderList[0].SetPointLights(pointLights1, pointLightCount1);
		}
		else if (mainWindow.getLuminaria() == 1 && mainWindow.getPastel() == 0) {
			shaderList[0].SetPointLights(pointLights, pointLightCount);
		}
		else if (mainWindow.getLuminaria() == 0 && mainWindow.getPastel() == 1) {
		...
		//Cofre
		model = glm::translate(model, glm::vec3(-0.05f, 0.38f, 1.33f));
		...
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-0.05f, 4.0f, 18.0f));
		model = glm::scale(model, glm::vec3(10.0f, 10.0f, 0.3f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		pisoTexture.UseTexture();
