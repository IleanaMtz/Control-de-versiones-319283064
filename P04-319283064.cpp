...
Teclas K, L, I y O para rotacion de ruedas
*/
#include <stdio.h>
#include <string.h>
#include<cmath>
...
int main()
{
	...
	*/
	...
	glm::mat4 centro(1.0);//Inicializar matriz de Modelo 4x4 auxiliar para la jerarquía
	glm::mat4 centroCabeza(1.0);//Inicializar matriz de Modelo 4x4 auxiliar para la jerarquía
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Ruedas
		model = glm::translate(model, glm::vec3(-2.2f, -4.0f, 2.5f));
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.28f, 0.28f, 0.28f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		...
		//Articulación 4 extremo del tercer brazo
		model = glm::translate(model, glm::vec3(2.5f, 0.0f, 0.0f));
		model = glm::rotate(model, glm::radians(-135.0f), glm::vec3(0.0f, 0.0f, 1.0f));
		...
		model = glm::translate(model, glm::vec3(20.0f, 4.0f, -4.0f));
		...
		glUniformMatrix4fv(uniformView, 1, GL_FALSE, glm::value_ptr(camera.calculateViewMatrix()));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.6863f, 0.3765f, 0.102f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		meshList[0]->RenderMesh();
		model = modelaux;
		...
		modelaux = model;
		model = glm::scale(model, glm::vec3(0.5f, 4.5f, 4.5f));
		color = glm::vec3(1.0f, 0.0f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		meshList[0]->RenderMesh();
		...
		//Hocico
		model = glm::translate(model, glm::vec3(-3.25f, -1.25f, 0.0f));
		modelaux = model;
		model = glm::scale(model, glm::vec3(1.5f, 2.5f, 5.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		meshList[0]->RenderMesh();
		model = modelaux;

		//Nariz
		model = glm::translate(model, glm::vec3(-0.26f, 0.76f, 0.0f));
		modelaux = model;
		model = glm::scale(model, glm::vec3(1.0f, 1.0f, 5.1f));
